...
import { staticMeals, getHighProteinMeals, getLowCalorieMeals, getLowCarbMeals, StaticMeal } from "@/libs/data/static-meals";

// Debug helper to get ms since page start
const debugNow = () => (typeof performance !== 'undefined' ? Math.round(performance.now()) : 0);
...
// LazyRender mounts children only when scrolled near viewport to load top-to-bottom progressively
const LazyRender = ({
  children,
  placeholder,
...
const MealCardSkeleton = ({ index = 0 }: { index?: number }) => (
  <Card className={`overflow-hidden hover:shadow-lg transition-shadow bg-white rounded-2xl border border-gray-100 relative ${
    index >= 1 ? 'hidden min-[420px]:block' : ''
  } ${
    index >= 2 ? 'hidden md:block' : ''
  } ${
    index >= 3 ? 'hidden min-[900px]:block' : ''
  } ${
    index >= 4 ? 'hidden min-[1100px]:block' : ''
  } w-[320px] min-[420px]:w-[191px] min-[600px]:w-[260px] md:w-[300px] min-[900px]:w-[280px] lg:w-[280px] min-[1100px]:w-[300px] h-[337px] min-[600px]:h-[380px] md:h-[420px] min-[900px]:h-[400px]`}>
    <div className="relative h-32 min-[600px]:h-40 md:h-48 bg-gray-50">
      <Skeleton className="w-full h-full" />
    </div>
    <CardContent className="p-3 min-[600px]:p-4 md:p-4 pb-12 min-[600px]:pb-14 md:pb-16">
    ...
    <div className="absolute bottom-3 left-3 right-3 min-[600px]:bottom-4 min-[600px]:left-4 min-[600px]:right-4 md:bottom-4 md:left-4 md:right-4 flex gap-2" style={{ height: '32px' }}>
      <Skeleton className="flex-1 h-8 min-[600px]:h-9 md:h-10 rounded-lg" />
      <Skeleton className="flex-1 h-8 min-[600px]:h-9 md:h-10 rounded-lg" />
    </div>
  </Card>
);
...
const MealCard = ({ 
  ...
  // For static meals, we'll simulate restaurant data
  const restaurantData = {
    _id: `restaurant-${meal.id}`,
    name: meal.restaurant,
    heroUrl: meal.restaurantLogo,
    distance: Math.round((Math.random() * 5 + 0.5) * 100) / 100, // Random distance between 0.5-5.5 miles
  };

  return (
    <Card 
      className={`overflow-hidden hover:shadow-lg transition-shadow bg-white rounded-2xl border border-gray-100 relative ${
        index >= 1 ? 'hidden min-[420px]:block' : ''
      } ${
        index >= 2 ? 'hidden md:block' : ''
      } ${
        ...
        index >= 4 ? 'hidden min-[1100px]:block' : ''
      } w-[320px] min-[420px]:w-[191px] min-[600px]:w-[260px] md:w-[300px] min-[900px]:w-[280px] lg:w-[280px] min-[1100px]:w-[300px] h-[337px] min-[600px]:h-[380px] md:h-[420px] min-[900px]:h-[400px] md:min-h-[400px]`} 
      style={{ 
    ...
    >
      <div className="relative h-32 min-[600px]:h-40 md:h-48 bg-gray-50">
        <Image
          src={meal.foodImage || imageFallback}
          alt={meal.name}
          fill
          sizes="(max-width: 768px) 100vw, 25vw"
          className="object-cover"
          onError={(e) => {
            console.error('Image failed to load:', meal.foodImage);
            console.error('Using fallback:', imageFallback);
          }}
          onLoad={() => {
            console.log('Image loaded successfully:', meal.foodImage);
          }}
        />
        {/* Restaurant Image */}
        {restaurantData?.heroUrl && (
          <div className="absolute top-2 right-2 w-8 h-8 rounded-full overflow-hidden border-2 border-white shadow-md">
            <Image
              src={restaurantData.heroUrl}
              alt={restaurantData.name || "Restaurant"}
              width={32}
              height={32}
              className="w-full h-full object-cover"
              onError={(e) => {
                console.error('Restaurant logo failed to load:', restaurantData.heroUrl);
              }}
              onLoad={() => {
                console.log('Restaurant logo loaded successfully:', restaurantData.heroUrl);
              }}
            />
          </div>
        )}
      ...
      <CardContent className="p-3 min-[600px]:p-4 md:p-4 pb-12 min-[600px]:pb-14 md:pb-16">
        ...
        <div className="h-6 md:h-8 mb-1 md:mb-2">
          <p className="text-base md:text-lg font-bold text-[#4E56D3]">{meal ? `$${(meal.price / 100).toFixed(2)}` : ""}</p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-1.5 md:hidden">
            <div className="flex items-center gap-1 bg-green-50 rounded-lg px-1.5 py-1">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[10px] font-extrabold text-gray-900">{meal?.fat ?? "-"}</div>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-yellow-50 rounded-lg px-1.5 py-1">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[10px] font-extrabold text-gray-900">{meal?.calories ?? "-"}</div>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-green-50 rounded-lg px-1.5 py-1">
              <div className="w-4 h-4 rounded-full bg-green-100 flex items-center justify-center">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[10px] font-extrabold text-gray-900">{meal?.protein ?? "-"}</div>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-blue-50 rounded-lg px-1.5 py-1">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[10px] font-extrabold text-gray-900">{meal?.carbs ?? "-"}</div>
              </div>
            </div>
          </div>
          ...
          <div className="hidden md:flex justify-between gap-1.5">
            <div className="flex items-center gap-1 bg-green-50 rounded-lg px-1.5 py-1 flex-1">
              ...
              <div className="leading-tight">
                ...
                <div className="text-xs font-extrabold text-gray-900">{meal?.fat ?? "-"}</div>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-yellow-50 rounded-lg px-1.5 py-1 flex-1">
              ...
              <div className="leading-tight">
                ...
                <div className="text-xs font-extrabold text-gray-900">{meal?.calories ?? "-"}</div>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-green-50 rounded-lg px-1.5 py-1 flex-1">
              <div className="w-5 h-5 rounded-full bg-green-100 flex items-center justify-center">
              ...
              <div className="leading-tight">
                ...
                <div className="text-xs font-extrabold text-gray-900">{meal?.protein ?? "-"}</div>
              </div>
            </div>
            <div className="flex items-center gap-1 bg-blue-50 rounded-lg px-1.5 py-1 flex-1">
              ...
              <div className="leading-tight">
                ...
                <div className="text-xs font-extrabold text-gray-900">{meal?.carbs ?? "-"}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* Action buttons */}
      <div className="absolute bottom-3 left-3 right-3 min-[600px]:bottom-4 min-[600px]:left-4 min-[600px]:right-4 md:bottom-4 md:left-4 md:right-4 flex gap-2" style={{ height: '32px' }}>
        <Button onClick={() => handleAddToProfile(meal?.id)} className="flex-1 bg-[#4E56D3] hover:bg-[#4046C7] text-white rounded-lg h-8 min-[600px]:h-9 md:h-10 text-sm min-[600px]:text-sm md:text-base">
          Add
        </Button>
        <Button onClick={() => handleCompareClick(meal?.id, meal?.name)} variant="ghost" className="flex-1 text-[#4E56D3] hover:bg-gray-50 rounded-lg border-none h-8 min-[600px]:h-9 md:h-10 text-sm min-[600px]:text-sm md:text-base">
          {firstCompareId === meal?.id ? "Selected" : "Compare"}
        </Button>
      </div>
    </Card>
  );
...
const HeroSection = () => {
  if (typeof window !== 'undefined') {
    console.log("HERO: render", debugNow(), "ms");
  }
  ...
  // When the filter popup closes, navigate to /app with URL params so the products page applies filters server-side
  useEffect(() => {
    const wasOpen = wasOpenRef.current;
    wasOpenRef.current = isSearchPopupOpen;
    if (wasOpen && !isSearchPopupOpen) {
      const params = new URLSearchParams();
      // Try to parse calorie as a single number; map to a small +/- range
      const cal = parseInt((searchQuery.calorie || '').replace(/[^0-9]/g, ''), 10);
      if (!isNaN(cal) && cal > 0) {
        const min = Math.max(100, cal - 100);
        const max = Math.min(3500, cal + 100);
        params.set('minCal', String(min));
        params.set('maxCal', String(max));
      }
      // Diets -> comma-separated ids (best-effort mapping)
      const dietRaw = (searchQuery.diet || '').toLowerCase();
      ...
      const dietTokens = dietRaw.split(',').map(s => s.trim()).filter(Boolean).map(d => dietMap[d] || d).filter(Boolean);
      if (dietTokens.length > 0) params.set('diets', dietTokens.join(','));
      // Allergies -> pass through as comma-separated
      const allergiesTokens = (searchQuery.allergies || '').split(',').map(s => s.trim()).filter(Boolean);
      if (allergiesTokens.length > 0) params.set('allergies', allergiesTokens.join(','));
      const url = params.toString().length > 0 ? `/app?${params.toString()}` : `/app`;
      console.log('HERO: navigate to filters', url);
      router.push(url);
    }
  }, [isSearchPopupOpen, searchQuery, router]);
        ...
        <div className="flex justify-center pb-0">
          <div className="w-full max-w-xs sm:max-w-sm md:max-w-md lg:max-w-lg xl:max-w-xl">
            <Image
              ...
              priority
              loading="eager"
              onLoad={() => console.log("IMG: hero loaded", debugNow(), "ms")}
            />
          </div>
        </div>
      </div>
...
// Helper function to get responsive item count
const getResponsiveItemCount = () => {
  if (typeof window === 'undefined') return 2; // SSR fallback
  
  const width = window.innerWidth;
  if (width >= 1100) return 4; // min-[1100px]:grid-cols-4
  if (width >= 900) return 3;  // min-[900px]:grid-cols-3
  if (width >= 768) return 2;  // md:grid-cols-2
  return 2; // 0-768px: show 2 items
};

// Meal Categories Section
const MealCategoriesSection = () => {
  console.log("MEAL-SECT: render", debugNow(), "ms");
  const categories = [
  ...
  const [responsiveItemCount, setResponsiveItemCount] = useState(2);
  const router = useRouter();

  // Update responsive item count on window resize
  useEffect(() => {
    console.log("MEAL-SECT: resize handler attached", debugNow(), "ms");
    const updateItemCount = () => {
      const count = getResponsiveItemCount();
      console.log("MEAL-SECT: responsiveItemCount", count, debugNow(), "ms");
      setResponsiveItemCount(count);
    };
    
    updateItemCount(); // Initial call
  ...
  const handleCompareClick = async (mealId: string, mealName?: string) => {
    console.log("COMPARE: start", { firstCompareId, mealId }, debugNow(), "ms");
    if (!firstCompareId) {
    ...
    try {
      ...
      console.log("COMPARE: fetch done", debugNow() - t0, "ms");
      if (!res.ok) {
  ...
  const lowCalMeals = useMemo(() => getLowCalorieMeals().slice(0, responsiveItemCount), [responsiveItemCount]);
  const lowCarbMeals = useMemo(() => getLowCarbMeals().slice(0, responsiveItemCount), [responsiveItemCount]);
  const highProteinMeals = useMemo(() => getHighProteinMeals().slice(0, responsiveItemCount), [responsiveItemCount]);
    ...
    <section className="pt-8 sm:pt-16 pb-4 sm:pb-6 bg-gray-50 w-full">
      <div className="w-full max-w-[1280px] mx-auto px-6 sm:px-6">
        ...
        <div className="mb-12 sm:mb-16 w-full max-w-[1280px] mx-auto">
          ...
          <div className="grid grid-cols-1 min-[420px]:grid-cols-2 min-[600px]:grid-cols-2 md:grid-cols-2 min-[900px]:grid-cols-3 lg:grid-cols-3 min-[1100px]:grid-cols-4 gap-4 sm:gap-6 md:gap-6 lg:gap-6 w-full max-w-[1280px] mx-auto justify-items-center">
            {lowCalMeals.map((meal: StaticMeal, index: number) => (
              <LazyRender key={meal.id} placeholder={<MealCardSkeleton index={index} />}>
                <MealCard
                  meal={meal}
                  index={index}
        ...
        <div className="mb-12 sm:mb-16 w-full max-w-[1280px] mx-auto">
          ...
          <div className="flex overflow-x-auto gap-4 pb-2 no-scrollbar">
            {restaurants.map((restaurant, index) => (
              <LazyRender key={index} placeholder={<MobileRestaurantCardSkeleton />}>
                <Card className="aspect-square p-4 text-center hover:shadow-lg transition-shadow cursor-pointer bg-white rounded-2xl border border-gray-100 flex flex-col justify-center w-[120px] h-[120px] md:w-[180px] md:h-[180px] lg:w-[200px] lg:h-[200px] flex-shrink-0">
                <div className="w-full flex-1 flex items-center justify-center mb-2">
                  <Image
                    src={restaurant.logo}
                    alt={restaurant.name}
                ...
                <p className="text-xs md:text-sm lg:text-base font-medium text-gray-700 truncate w-full">{restaurant.name}</p>
              </Card>
              </LazyRender>
            ))}
          </div>
        ...
        <div className="mb-12 sm:mb-16 w-full max-w-[1280px] mx-auto">
          ...
          <div className="grid grid-cols-1 min-[420px]:grid-cols-2 min-[600px]:grid-cols-2 md:grid-cols-2 min-[900px]:grid-cols-3 lg:grid-cols-3 min-[1100px]:grid-cols-4 gap-4 sm:gap-6 md:gap-6 lg:gap-6 w-full max-w-[1280px] mx-auto justify-items-center">
            {lowCarbMeals.map((meal: StaticMeal, index: number) => (
              <LazyRender key={meal.id} placeholder={<MealCardSkeleton index={index} />}>
                <MealCard
                  meal={meal}
                  index={index}
        ...
        <div className="mb-12 sm:mb-16 max-w-7xl mx-auto w-full max-w-[1280px]">
          ...
          <div className="grid grid-cols-1 min-[420px]:grid-cols-2 min-[600px]:grid-cols-2 md:grid-cols-2 min-[900px]:grid-cols-3 lg:grid-cols-3 min-[1100px]:grid-cols-4 gap-4 sm:gap-6 md:gap-6 lg:gap-6 w-full max-w-[1280px] mx-auto justify-items-center">
            {highProteinMeals.map((meal: StaticMeal, index: number) => (
              <LazyRender key={meal.id} placeholder={<MealCardSkeleton index={index} />}>
                <MealCard
                  meal={meal}
                  index={index}
        ...
        <div className="sm:hidden">
          <div className="bg-gradient-to-br from-[#4E56D3] to-[#8B5CF6] rounded-3xl p-6 text-white">
            ...
            <p className="text-sm mb-6 opacity-90 leading-relaxed text-center">
              Healthy eating on the go is hard. LiteBite uses AI to find meals that match your goals, diet, and allergies—so you can eat out and stay on track.
            </p>
